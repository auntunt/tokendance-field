...
//
// 用法：
//   npx tsx scripts/discover-companies.ts              # 联网跑全部渠道
//   npx tsx scripts/discover-companies.ts --cache-only # 不联网，只用已缓存的响应重算
//   npx tsx scripts/discover-companies.ts --gh scaleai,databricks  # 额外补几个 Greenhouse 招聘板
//
// 产物：
...
// 岗位名是最硬的一种证据，值得为它多跑一次。
{
  ...
  // 不是「这家公司是谁」——和 fetch-filings.ts 里的 CIK 表同一个道理。
