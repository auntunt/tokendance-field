...
//
// 用法：
//   node scripts/fetch-filings.mjs                 # 抓全部已配代码的公司
//   node scripts/fetch-filings.mjs --only 002230    # 只抓一家（调试用）
//   node scripts/fetch-filings.mjs --cache-only     # 不联网，只用已下载的原文重抽
//
// 产物：
...
const total = results.reduce((sum, item) => sum + Object.values(item.facts).reduce((n, dim) => n + Object.keys(dim).length, 0), 0);
const holderTotal = holderResults.reduce((sum, item) => sum + Object.values(item.facts).reduce((n, dim) => n + Object.keys(dim).length, 0), 0);
