...
//
// 用法：
//   npm run report:build                               # 用默认语料路径
//   npm run report:build -- --corpus /path/data.json
//   npm run report:build -- --no-corpus                # 只出名单（语料不在手边时）
//
...
// 和 tests/build-kernel.ts 用同一个编译方式：直接调真实 TS 函数，
...
if (filings) {
  const count = filings.companies.reduce((sum, item) =>
    sum + Object.values(item.facts as Record<string, object>).reduce((n, dim) => n + Object.keys(dim).length, 0), 0);
