...
//
// 用法：
//   npx tsx scripts/seed-fde-signals.ts \
...
function makeSignals(): any[] {
  const out: any[] = [];
  const added = { investor: 0, holder: 0, customer: 0, fde: 0 };
...
const curatedRaw: any = existsSync(curatedPath) ? JSON.parse(readFileSync(curatedPath, "utf8")) : { signals: [], dropIds: [] };
const curatedSignals: any[] = Array.isArray(curatedRaw) ? curatedRaw : Array.isArray(curatedRaw.signals) ? curatedRaw.signals : [];
...
const existingSignals: any[] = Array.isArray(current.signals) ? current.signals : [];
...
const byId = new Map<string, any>(preserved.map(signal => [String(signal.id), signal]));
