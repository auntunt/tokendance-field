...
test("生产镜像可从真实来源样本建立不覆盖旧数据的 FDE 验收数据库", () => {
  ...
  try {
    ...
    assert.equal(db.prepare("SELECT name FROM company WHERE id='002410.SZ'").get().name, "广联达科技股份有限公司");
    assert.equal(db.prepare("SELECT count(*) AS count FROM industry_update").get().count, 0);
    db.close();
