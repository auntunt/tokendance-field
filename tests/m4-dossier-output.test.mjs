...
function seed() {
  ...
  for (const field of ["name", "upstream", "downstream", "kpis", "regulators"]) addFact("s-p13", "industry", "construction", field);
  db.prepare("INSERT INTO company VALUES ('002410.SZ', '广联达科技股份有限公司', 'construction', '', '深交所 002410')").run();
  for (const field of ["name", "industry_id", "listing"]) addFact("s-p13", "company", "002410.SZ", field);
  db.prepare("INSERT INTO industry_term VALUES ('term-bim', 'construction', 'BIM', '用可计算的三维模型承载工程数据', '')").run();
  for (const field of ["industry_id", "term", "plain_meaning"]) addFact("s-p13", "industry_term", "term-bim", field);
  const lines = [
  ...
  for (const [lineId, name, stepId, seq, step, owner, pain, sourceId] of lines) {
    ...
    for (const field of ["company_id", "name"]) addFact(sourceId, "business_line", lineId, field);
    for (const field of ["business_line_id", "seq", "name", "owner_org_unit", "pain_point"]) addFact(sourceId, "process_step", stepId, field);
  }
  const systems = [
  ...
  for (const [id, category, product, vendor, covers, sourceId] of systems) {
    ...
    for (const field of ["company_id", "category", "product", "vendor", "covers_process_step"]) addFact(sourceId, "system_in_use", id, field);
  }
  db.prepare("INSERT INTO org_unit VALUES ('G-ORG1', '002410.SZ', '董事会 / 董事长', NULL, NULL)").run();
  for (const field of ["company_id", "name"]) addFact("s-p14", "org_unit", "G-ORG1", field);
  ...
  for (const field of ["name", "bio", "stance"]) addFact("s-p14", "person", "G-P1", field);
  for (const field of ["company_id", "person_id", "title", "owns"]) addFact("s-p14", "position", "G-POS1", field);
  for (const [year, revenue, profit, rnd] of [[2023, 6524575067.70, 115837537.09, 1963000000], [2024, 6202873989.82, 250424298.94, 1805325661.16], [2025, 6068493454.08, 405114040.23, 1624692445.77]]) {
    ...
    for (const field of ["company_id", "year", "revenue", "net_profit", "rnd_expense"]) addFact("s-p14", "financial_snapshot", id, field);
  }
  db.prepare("INSERT INTO event VALUES ('G-E1', '002410.SZ', '2025-09-16', 'strategy', '发布 GCCP7.0 与建筑企业 AI 应用行动指南')").run();
  for (const field of ["company_id", "occurred_at", "kind", "summary"]) addFact("s-p14", "event", "G-E1", field);
  return { db, addFact };
}
...
test("生成可独立打开的 0–10 章 HTML 档案并保留来源链接", () => {
  ...
  assert.match(html, /class="sourced"/);
...
test("真实档案生成链路会持久化每次 DossierRun，并在下一次显示新增", () => {
  const { db, addFact } = seed();
  const first = generateDossier(db, "002410.SZ", "2026-09-01T08:00:00+08:00");
  assert.deepEqual(first.changes, []);
  assert.equal(db.prepare("SELECT count(*) AS count FROM dossier_run").get().count, 1);
  assert.equal(db.prepare("SELECT count(*) AS count FROM opportunity").get().count, 6);
  db.prepare("INSERT INTO event VALUES ('G-E3', '002410.SZ', '2026-09-08', 'statement', '新增行业模型评测动作')").run();
  for (const field of ["company_id", "occurred_at", "kind", "summary"]) addFact("s-p13", "event", "G-E3", field);
  const second = generateDossier(db, "002410.SZ", "2026-09-08T08:00:00+08:00");
  assert.equal(db.prepare("SELECT count(*) AS count FROM dossier_run").get().count, 2);
  assert.ok(second.changes.some(change => change.table === "event" && change.rowId === "G-E3"));
  assert.match(second.html, /自上次以来的变化/);
  assert.match(second.html, /新增行业模型评测动作/);
  db.close();
});

test("渲染时直接拒绝超过章节文字上限或多句的内容", () => {
  const { db, addFact } = seed();
  const opportunities = generateOpportunities(db, "002410.SZ");
  persistOpportunities(db, opportunities);
  const prep = generateEntryPrep(db, "002410.SZ", opportunities);
  const tooLong = structuredClone(prep);
  tooLong.questions.push({ question: "问".repeat(401), basisIds: ["G-PS1"] });
  assert.throws(() => renderDossierHtml(db, "002410.SZ", opportunities, tooLong, []), /第 10 章.*超过 400 字/);
  db.prepare("INSERT INTO event VALUES ('G-E4', '002410.SZ', '2026-09-09', 'statement', '第一句。第二句。')").run();
  addFact("s-p13", "event", "G-E4", "summary");
  assert.throws(() => renderDossierHtml(db, "002410.SZ", opportunities, prep, []), /第 6 章.*超过 1 句/);
  db.close();
});

test("同行业第二家客户直接复用同一 Industry 与 IndustryTerm", () => {
  const { db, addFact } = seed();
  db.prepare("INSERT INTO company VALUES ('SECOND', '同行业第二家', 'construction', '', '未上市')").run();
  for (const field of ["name", "industry_id", "listing"]) addFact("s-p13", "company", "SECOND", field);
  assert.equal(db.prepare("SELECT count(*) AS count FROM industry WHERE id='construction'").get().count, 1);
  assert.equal(db.prepare("SELECT count(*) AS count FROM industry_term WHERE industry_id='construction'").get().count, 1);
  const html = renderDossierHtml(db, "SECOND", [], { stakeholders: [], questions: [], risks: [] }, []);
  assert.match(html, /建筑产业数字化/);
  assert.match(html, /BIM/);
  db.close();
});
