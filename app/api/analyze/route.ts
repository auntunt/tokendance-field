...
type RuntimeEnv = { OPENAI_API_KEY?: string; ANTHROPIC_API_KEY?: string; DEEPSEEK_API_KEY?: string };
...
export async function POST(request: Request) {
  try {
    ...
    if (!body.provider || !body.model || !body.title || !body.evidence) return Response.json({ error: "模型、标题与原始依据均为必填项" }, { status: 400 });
    ...
    if (body.provider === "anthropic") {
      upstream = await fetch(body.endpoint?.trim() || "https://api.anthropic.com/v1/messages", { method: "POST", signal: abort, headers: { "content-type": "application/json", "x-api-key": apiKey, "anthropic-version": "2023-06-01" }, body: JSON.stringify({ model: body.model, max_tokens: 1200, system: systemPrompt, messages: [{ role: "user", content: userInput }] }) });
    } else if (body.provider === "openai") {
      upstream = await fetch(body.endpoint?.trim() || "https://api.openai.com/v1/responses", { method: "POST", signal: abort, headers: { "content-type": "application/json", authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model: body.model, instructions: systemPrompt, input: userInput, text: { format: { type: "json_object" } }, store: false }) });
    } else {
      const endpoint = body.endpoint?.trim() || (body.provider === "deepseek" ? "https://api.deepseek.com/chat/completions" : "");
      if (!endpoint) return Response.json({ error: "兼容服务需要填写完整的 Chat Completions 地址" }, { status: 400 });
      upstream = await fetch(endpoint, { method: "POST", signal: abort, headers: { "content-type": "application/json", authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model: body.model, messages: [{ role: "system", content: systemPrompt }, { role: "user", content: userInput }], response_format: { type: "json_object" }, temperature: 0.2 }) });
    }
    ...
    return Response.json({ analysis, provider: body.provider, model: body.model, generatedAt: new Date().toISOString() });
  } catch (error) {
