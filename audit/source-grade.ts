...
/** 结构性后缀：不可伪装，优先级最高。 */
type SourceGrade = "independent" | "related" | "unknown";
type GradeResult = { type: SourceGrade; reason: string };

const STRUCTURAL: Array<[RegExp, SourceGrade, string]> = [
  [/\.gov\.cn$|\.gov$/, "independent", "政府域名（行政备案/政府目录）"],
  [/\.edu\.cn$/, "independent", "高校域名（第三方，非企业自述）"],
];

/** 站点类型表。同一类型的判级理由必须一致。 */
const BY_TYPE: Array<[SourceGrade, string, string[]]> = [
...
function matchHost(host: string, list: string[]): boolean {
  return list.some(h => host === h || host.endsWith(`.${h}`));
}

/** 子域名比父域名更具体，必须优先。否则 pitchhub.36kr.com（项目方自填）
 *  会先命中 36kr.com（第三方媒体）而被误判为 independent。 */
function bestMatch(host: string): (GradeResult & { h: string }) | null {
  let best: (GradeResult & { h: string }) | null = null;
...
export function gradeUrl(raw: unknown): GradeResult {
...
export function gradeSources(sources: unknown[]): { type: SourceGrade; graded: GradeResult[] } {
