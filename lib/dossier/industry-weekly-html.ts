...
export function renderIndustryWeeklyHtml(db: DossierDatabase, industryId: string, from: string, to: string): string {
  ...
  const selectedCount = rows.filter(row => row.promoted_to_event_id).length;
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(industry.name)}行业周报</title><style>:root{color-scheme:dark}body{max-width:1180px;margin:48px auto;padding:0 24px;background:#0b0e12;color:#eef2f7;font:14px/1.6 system-ui}h1{font-size:40px}p{color:#95a1b2}.progress{display:inline-block;margin-left:10px;padding:2px 8px;border:1px solid #ffb224;color:#ffb224}table{width:100%;border-collapse:collapse;background:#11161d}th,td{padding:14px;border:1px solid #28313d;text-align:left;vertical-align:top}th{color:#ffb224}a{color:#65c7ff}button{border:1px solid #ffb224;background:transparent;color:#ffb224;padding:6px 10px;cursor:pointer}code{color:#72d3ff}</style></head><body><h1>${escape(industry.name)}行业周报</h1><p>${escape(from)} — ${escape(to)} · ${rows.length} 条 <span class="progress">已选择 ${selectedCount} / 至少 3 条</span></p><table><thead><tr><th>日期</th><th>类型</th><th>条目</th><th>状态</th><th>来源</th></tr></thead><tbody>${body}</tbody></table></body></html>`;
}
